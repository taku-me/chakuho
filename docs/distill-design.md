...
**訓練した課題(AX 木の要素選択)から離れるほど落ち、行動ラベルの選択では転移しない。**
いずれも教師 27B との一致率:
...
| Ableton コマンド解析 1,550 問 | 日本語 1 行 → track/clip/scene | **74%** | — |
| jev-mario 20 問 | 盤面 → 行動ラベル 9 択 | **0%** | **0%** |

**mario の 0 点は蒸留の劣化ではない。未学習の素も 0/20 で、1.7B の天井。**
答えが 2 種類に潰れる(学生 `run and jump right` 15 / `jump right` 5、素 `jump right` 19)。
**GUI 以外の判定は 27B に残すこと。**
